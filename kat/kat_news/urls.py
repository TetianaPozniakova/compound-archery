...
urlpatterns = patterns('',
    url(r'^(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})-(?P<slug>[-\w]+)/$',
        DateDetailView.as_view(model=News, date_field="date", month_format='%m'), name='news_detail'),
)
...
if news_settings.ENABLE_NEWS_DATE_ARCHIVE:
    urlpatterns += patterns('',
        ...
        url(r'^archive/(?P<year>\d{4})-(?P<month>\d{2})/$', NewsMonthArchiveView.as_view(month_format='%m'),
            name='news_archive_month'),
        url(r'^archive/(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})/$', NewsDayArchiveView.as_view(),
